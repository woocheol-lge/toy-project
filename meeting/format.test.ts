...
import { addItem, createEmptyMeeting, setConclusion } from "./meeting-state";
...
describe("결론 마크다운", () => {
  ...
  test("주제 제목과 결론을 함께 담는다", () => {
    const markdown = toMarkdown(meetingWithConclusions());

    expect(markdown).toContain("## 배포 일정");
    expect(markdown).toContain("다음 주 화요일 배포로 확정");
  });

  test("결론이 빈 주제도 제목은 담고 결론 없음을 표시한다", () => {
    const markdown = toMarkdown(meetingWithConclusions());

    expect(markdown).toContain("## 채용");
    expect(markdown).toContain("_결론 없음_");
  });
